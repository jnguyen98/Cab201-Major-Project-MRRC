    ...
    public class InnerSubInterfaceCRM : SubInterface
    {
        ...
        private void DOBValidationInput()
        {
            ...
            do
            {
                ...
                else
                {
                    ...
                    string ActualAge = int.Parse(dateA.Substring(6)).ToString();
                    string AgeConstraint = (int.Parse(dateB.Substring(5)) - 18).ToString();
                    if (dateA.CompareTo(dateB) == 1)
                    {
                        Console.WriteLine("\nYou entered {0}, which not a valid Date of Birth.", dateA);
                        Console.WriteLine("Please enter a Date which is less than or equal to {0}\n", dateB);
                        dobIsValid = false;
                    }
                    // If the age is not 18 years old, then dob isnt valid
                    else if (ActualAge.CompareTo(AgeConstraint) == 1)
                    {
                        Console.WriteLine("\nYou entered {0}, which not a valid Date of Birth.", dateA);
                        Console.WriteLine("Customer has to be at least 18 years old.\n");
                        dobIsValid = false;
                    }
                }
        ...
        private void ModifyIDValidationInput()
        {
            ...
            do
            {
                ...
                IDisValid = int.TryParse(id, out ModifiedID);
                if (IDtoMofify < 0)
                {
                    Console.WriteLine("\nYou entered '{0}', which is not a positive integer", ModifiedID);
                    Console.WriteLine("Enter an ID which is a positive integer\n");
                    IDisValid = false;
                }
                else if (!int.TryParse(id, out ModifiedID))
                {
                    Console.WriteLine("\nYou entered '{0}', which is not an integer", ModifiedID);
                    Console.WriteLine("Enter an ID which is an integer\n");
                    IDisValid = false;
                }
                else if (Crm.CustomerList.Any(archivedCustomer => archivedCustomer.ID == ModifiedID))
                {
                    Console.WriteLine("\nID number '{0}' cannot be accepted as it exists in the customer list", ModifiedID);
        ...
        public void ModifyCustomerSubmenu(bool clear = true)
        {
            ...
            if (customer != null)
            {
                ...
                // Set the customer ID to -1, in case the customer wants to retain his/her ID and only wants
                ...
                int changeForRetain = -1;
                customer.ID = changeForRetain;
                ModifyIDValidationInput();
                customer.ID = ModifiedID;
                Console.Write("Title*: ");
                title = StringToTitleCase(Console.ReadLine());
                ...
                Console.Write("\nSuccessfully modified customer '{0} - {1} {2} {3}' to '{4} {5} {6} {7}' within the customers list",
                           OldID, OldTitle, OldFirstName, OldLastName, ModifiedID, title, firstName, lastName);
                Crm.SaveToFile();
