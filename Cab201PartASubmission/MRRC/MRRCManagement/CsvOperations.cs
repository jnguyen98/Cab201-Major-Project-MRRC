...
namespace MRRCManagement
{
    public class CsvOperations
    {
        ...
        // This method creates the rental data table
        public static void RentalsToTable(Fleet fleet, DataTable Name)
        {
            ...
            // Implement loop to store all the rental lines into the variable
            for(int i = 0; i < fleet.RentalListRego.Count; i++)
            {
                totalLines.Add(fleet.RentalListRego[i] + "," + fleet.RentalListID[i].ToString() + "," + fleet.DailyCost[i].ToString("C"));
            }
        ...
        #region Read a record attribute
        // This method reads a search term of a record and returns an output term, which is
        // another field attribute but exists in the same record. E.g. Search for ID '10' and return the firstName 
        public static string ReadRecordAttribute(string searchTerm, string filePath, int positionOfSearchTerm, int positionOfOutputTerm)
        {
            string recordNotFound = "Record attribute not Found";
            ...
            try
            {
                ...
                for (int i = 0; i < lines.Length; i++)
                {
                    ...
                    if (RecordExists(searchTerm, fields, positionOfSearchTerm))
                    {
                        return fields[positionOfOutputTerm];
                    }
                }
            ...
            {
                throw new Exception("Could not read record", error);
            };
        }
        #endregion
